...
{                                 //argv[]는 외부에서 입력한 문자열들  
 ...
 for(y = 0; y < YY; y++)       //반복문과 if문을 사용하여
  for(x = 0; x < XX; x++)      //명암도가 100 이하는 명암도를 0으로
   if(image[y][x] <= 100)       //명암도가 100 초과는 명암도를 255로 이진화 함
    image[y][x] = 0;
   else
    image[y][x] = 255;
 ...
 for(y = 0; y < YY; y++){
  for(x = 0; x < XX; x++){    //반복문을 이용하여 이진화 시킨 픽셀 값을
   fprintf(file2,"%4d", image[y][x]);  //세 번째 문자열 파일에 입력
  }
  fprintf(file2, "\n");
 }
 return 0;
