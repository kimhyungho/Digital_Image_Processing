...
{                                  //argv[]는 외부에서 입력한 문자열들
 ...
 for(y = 0; y < YY; y++)       //반복문을 사용하여 Blue 분리
  for(x = 2; x < XX*3; x+=3)   //B-Channel
 ...
 for(y = 0; y < YY; y++){
  for(x = 0; x < XX*3; x++){   //컬러영상은 픽셀 당 값 개를 가지므로 *3
   fprintf(file2,"%4d", image2[y][x]);  //반복문을 이용하여 Blue로 분리한
  }                                    //픽셀 값을 세 번째 문자열 파일에 입력
  fprintf(file2, "\n");
 }
 return 0;
}
